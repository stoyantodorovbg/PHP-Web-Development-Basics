...
class Carshop
{
    ...
    protected function getSales()
    {
        ...
        `cars`.`model`,
        `cars`.`year`,
        `sales`.`date_time` AS `date_of_deal`,
        `sales`.`amount`
        FROM `sales`
        INNER JOIN `cars`
        ...
        //$result = $this->db->prepare("SELECT `get_sales()`");
        //$result->execute();
        //while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        //    $out .= '----------'.PHP_EOL.'Total: '.$row['get_sales()'];
        //}

        $exc_line = $this->db->prepare("SELECT `get_sales()` FROM `sales`");
        $exc_line->execute();
        $exc_line_data = $exc_line->fetch();
        $out .= '----------'.PHP_EOL.'Total: '.$exc_line_data;
